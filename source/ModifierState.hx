...
class ModifierState extends MusicBeatState
{
	...
	function refreshModifiers():Void
	{
			...
			for (i in 0...modifierList.length)
			{
				...
				menuItem.frames = Paths.getSparrowAtlas('modifiers/' +  modifierList[i].name);
    ...
    override function update(elapsed:Float)
        {
			...
			if (fakeMP < 0.1 && !ModifierState.modifierList[0].value) //practice shit be like
				fakeMP = 0.1;
			...
			if (selectable && !goingBack && !substated)
			{
				...
				if (controls.BACK)
				{
					if (FreeplayState.infreeplay != true && StoryMenuState.instorymenu != true) {
						FlxG.switchState(new PlayMenuState());	
					}
					else if (FreeplayState.infreeplay == true)
					{
						FlxG.switchState(new FreeplayState());
					}
					else if (StoryMenuState.instorymenu == true)
					{
