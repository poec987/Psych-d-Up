...
class ChartTypeSubstate extends MusicBeatSubstate
{
        ...
        menuItems.forEach(function(spr:FlxSprite)
            {
                if (spr.ID == curSelected && !selectedSomethin && selectable)
                {
                    ...
                    camFollow.x = 0;
                    spr.x = FlxMath.lerp(spr.x, -1300, camLerp/(ClientPrefs.framerate / 60));
                }

                spr.x = FlxMath.lerp(spr.x, 600, camLerp/(ClientPrefs.framerate / 60));
            });
    }
